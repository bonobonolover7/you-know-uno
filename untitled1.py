...
<style>
    ...
    /* 카드 즉시 추가 모션 */
    .card-in { animation: slideUp 0.3s ease-out; }
...
if 'initialized' not in st.session_state:
    ...
    st.session_state.update({"deck": deck, "players": players, "discard": [first], "current_color": first.color, "turn": 0, "direction": 1, "stack": 0, "game_msg": "🎮 배틀 시작!", "initialized": True, "effect": None, "v_color": None})
...
def draw_to_hand(p_idx):
    ...
    st.session_state.game_msg = f"🃏 {p['name']}님이 {count}장을 가져왔습니다."
    # 턴 넘기기 전 강제 갱신으로 카드 들어온 걸 먼저 보여줌
    st.rerun()

# ---------------- 4. UI 렌더링 ----------------
# 1순위: 효과 오버레이 (독립 레이어라 배경을 안 지움)
eff = st.session_state.get('effect')
v_col = st.session_state.get('v_color')
if eff:
    st.markdown(f'<div class="vignette" style="--v-color: {{"Red":"#FF4B4B","Yellow":"#F4C542","Green":"#4CAF50","Blue":"#2196F3"}.get(v_col, "#333")};"></div>', unsafe_allow_html=True)
...
with mid:
    st.markdown(f"<center>현재 색상: <b style='color:{v_col}'>{st.session_state.current_color}</b></center>", unsafe_allow_html=True)
    top_card = st.session_state.discard[-1]
    # 바닥 카드 렌더링
    st.markdown(f"""<div style="background:{{"Red":"#FF4B4B","Yellow":"#F4C542","Green":"#4CAF50","Blue":"#2196F3"}.get(top_card.color, "#333")}; width:120px; height:180px; border-radius:15px; border:4px solid white; margin:auto; display:flex; align-items:center; justify-content:center; color:white; font-size:40px; font-weight:bold; box-shadow:0 10px 20px rgba(0,0,0,0.2);">{top_card.value if not "Wild" in top_card.color else "🌈"}</div>""", unsafe_allow_html=True)
    if st.session_state.stack > 0: st.error(f"누적 공격: +{st.session_state.stack}")
...
if st.session_state.get('waiting_color'):
    ...
    for i, c in enumerate(curr_p["hand"]):
        with h_cols[i]:
            card_color = {"Red":"#FF4B4B","Yellow":"#F4C542","Green":"#4CAF50","Blue":"#2196F3"}.get(c.color, "#333")
            st.markdown(f'<div class="card-in" style="background:{card_color}; height:120px; border-radius:8px; border:2px solid white; display:flex; align-items:center; justify-content:center; color:white; font-weight:bold; box-shadow:0 4px 8px rgba(0,0,0,0.1);">{c.value if "Wild" not in c.color else "🌈"}</div>', unsafe_allow_html=True)
    ...
    if playable:
        ...
        draw_to_hand(st.session_state.turn)
        st.session_state.turn = (st.session_state.turn + st.session_state.direction) % 4
    st.rerun()
